<x-layout>
    ...
    <div class="mx-auto max-w-screen-md grid md:grid-cols-1 gap-6 p-4">
        <x-bladewind::card class="">
            <h1
                class="mb-4 text-4xl font-extrabold leading-none tracking-tight text-green-600 dark:text-green-500 md:text-5xl lg:text-6xl">
                {{ucfirst($username)}} <span class=" text-gray-900">'s</span>
            </h1>
    ...
    <div class="mx-auto max-w-screen-md grid md:grid-cols-1 gap-6 px-4 py-8">
        ...
        <x-bladewind::card class="">
            ...
            <div class="grid grid-cols-7 gap-1 rounded-full">
                ...
                <div @if($checkinForDay) data-inverted data-tooltip="{{ getTime($checkinForDay->created_at) }}" @endif
                    id="day"
                    class="text-center border rounded-full aspect-square flex flex-col justify-center relative md:w-20 md:h-20 w-14 h-14 {{$checkinForDay == null && $isBeforeOrEqualToday && $weekend==false ? 'bg-blue-100 text-blue-800 border-blue-400' : $bgColor }}">
